...
server <- function(input, output, session) {
  ...
  master_data <- reactive({
    ...
    # -----------------------------
    combined <- read_excel(master_path, sheet = "Combined_Model_Results") %>%
      mutate(
        DSM2_Node         = as.character(DSM2_Node),
        Forecast_scenario = as.character(Forecast_scenario),
    ...
    # -----------------------------
    node_meta <- read_excel(master_path, sheet = "PTM_All_Scenarios") %>%
    ...
    forecast_scenario_choices <- combined %>%
      filter(PTM_scenario == "Forecast_7day_average") %>%
  ...
  tab_to_ptm_scenario <- function(tabname) {
    if (grepl("^current7", tabname)) {
      "Measured_7day_average"
    } else if (grepl("^current30", tabname)) {
      "Measured_30day_average"
  ...
  active_results <- reactive({
    ...
    md$combined %>%
      filter(PTM_scenario == ptm_scenario, Forecast_scenario == scen)
  })
  ...
  make_summary_table <- function(model_name) {
    ...
    data.frame(
      ...
      Region            = sel$Region,
      Scenario          = sel$PTM_scenario,
      Forecast_Scenario = sel$Forecast_scenario,
