...
event_horizon_tab <- function(tab_name, title, theme_class, map_id, scatter_id) {
  tabItem(
    tabName = tab_name,
    ...
    div(
      ...
      fluidRow(
        box(
          ...
          status = NULL,
          solidHeader = FALSE,
          leafletOutput(map_id, height = 650)
        ),
        
        box(
          width = 4,
          title = paste(title, "- Scatter Plot"),
          status = NULL,
          solidHeader = FALSE,
          plotOutput(scatter_id, height = 650)
        )
      )
    )
  )
